...
| M05 | 通用内容与媒体基础 | 动态、相册等内容模块的公共能力 | 上传校验、媒体元数据、内容状态、软删除、匿名展示工具 | M02、M03 | 阶段 3 |
| M06 | 首页动态与评论 | 站内日常分享和互动基础 | 动态、图片、标签、置顶、评论、回复、删除、匿名展示 | M03、M05 | 阶段 4 |
...
| M14 | 系统通知预留 | 系统提醒能力，不包含私信 | 审核结果、公告、评论回复、活动状态等系统通知预留 | M02、M06、M07、M08 | 阶段 7，可按需裁剪 |
...
动态、相册和照片都依赖媒体上传、内容状态、软删除和匿名展示能力，因此建议在具体内容模块之前先抽象公共能力。
...
| 匿名展示工具 | 根据 display_mode 和当前用户角色返回展示身份 | 权限与账号 | 必须先于动态、评论、生日祝福 |

阶段目标：业务内容模块可以复用统一的上传、内容状态、软删除和匿名展示能力。
...
| 评论与回复模型 | 支持动态评论、回复、删除和状态管理 | 动态模型、匿名展示 | 可与动态接口并行设计 |
| 匿名展示接入 | 动态、评论按普通用户和管理员视角返回不同作者信息 | 匿名展示工具 | 依赖公共能力 |
| 置顶和标签 | 支持常用分类、按时间排序、置顶展示 | 动态基础 API | 可后置 |
| 用户删除 | 用户删除自己的普通动态和评论 | 动态和评论模型 | 依赖状态规则 |
| 前端动态页面 | 动态列表、发布框、详情、评论区、图片展示 | 动态和评论 API | 可按接口契约并行开发 |

阶段目标：已审核同学可以在班级内部发布动态、评论互动，并遵守匿名展示和内容可管规则。
...
生日模块依赖资料中的生日月份和内容匿名展示能力。该模块功能轻量，可以与相册并行。
...
| 生日祝福 | 发布祝福留言，支持匿名展示 | 匿名展示工具 | 可与本月生日 API 并行 |
...
| 任务项 | 说明 | 前置依赖 | 可并行性 |
| --- | --- | --- | --- |
| 通知模型预留 | 接收人、类型、标题、内容、状态 | 账号模块 | 可后置 |
...
阶段目标：保留系统通知能力，但明确不提供私信、小群或聊天能力。
...
- 资料与通讯录可以和媒体基础并行。
- 资料编辑页面可以和通讯录页面并行。
- 媒体上传安全、内容状态、匿名展示工具可以并行推进。

阶段完成标志：用户资料、隐私设置、通讯录、生日月份基础可用；媒体上传和匿名展示等公共能力可被动态、相册、生日祝福复用。
...
- 生日祝福管理可以与举报治理模块联动后再完善。

阶段完成标志：相册、照片、照片评论、本月生日同学和生日祝福可用，并符合上传安全、匿名展示和生日最小展示规则。
...
- 通知预留、移动端适配、错误提示和文档整理可以并行。
- 如果第一版时间紧，通知中心可以只保留后端模型或暂缓完整页面。

阶段完成标志：关键用户路径体验顺畅，系统通知不突破“禁止私信和小群”的产品边界。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 基础架构 | 工程基础设施 | 后端/前端/环境 | 建立 Django + DRF 后端基础、Vue 3 前端基础、MySQL/Redis 开发环境、OpenAPI 能力 | 无 | 不在本文档估算 | 阶段 1 | 阶段 1 | 待分配 | Backend / Web / DevOps | 工程结构如果频繁变更会影响后续模块 | 未开始 |
| 账号体系 | 注册登录与审核 | 身份可信准入 | 实现注册、登录、JWT、真实姓名、邀请码、审核状态和管理员审核 | 基础架构 | 不在本文档估算 | 阶段 2 | 阶段 2 | 待分配 | Backend / Web | 自定义用户模型若设计不当，后续迁移成本高 | 未开始 |
| 权限隐私 | 角色与访问控制 | 安全底座 | 实现角色、账号状态、管理接口权限、对象级权限、隐私字段过滤和统一错误码 | 账号体系 | 不在本文档估算 | 阶段 2 | 阶段 2 | 待分配 | Backend / Web | 权限遗漏会导致敏感数据泄露 | 未开始 |
| 用户资料 | 资料与通讯录 | 同学资料维护 | 实现个人资料、通讯录、联系方式可见范围、生日月份设置 | 账号体系、权限隐私 | 不在本文档估算 | 阶段 3 | 阶段 3 | 待分配 | Backend / Web | 联系方式默认不公开规则必须严格落实 | 未开始 |
| 内容基础 | 媒体与匿名展示 | 公共内容能力 | 实现上传校验、媒体元数据、内容状态、软删除、匿名展示工具 | 权限隐私 | 不在本文档估算 | 阶段 3 | 阶段 3 | 待分配 | Backend / Web | 上传安全和匿名作者过滤风险较高 | 未开始 |
| 社区互动 | 动态与评论 | 首页内容流 | 实现动态、图片、标签、置顶、评论、回复、删除和匿名展示 | 内容基础、权限隐私 | 不在本文档估算 | 阶段 4 | 阶段 4 | 待分配 | Backend / Web | 匿名展示不能泄露真实作者 | 未开始 |
| 活动组织 | 聚会/投票/接龙 | 活动实名参与 | 实现活动主体、报名、投票、接龙、状态管理、实名快照 | 资料与通讯录、权限隐私 | 不在本文档估算 | 阶段 4 | 阶段 4 | 待分配 | Backend / Web | 活动参与必须实名，不能出现匿名参与入口 | 未开始 |
| 公告 | 公告与置顶 | 班级重要信息 | 实现公告发布、置顶、有效期和已读确认 | 账号体系、权限隐私 | 不在本文档估算 | 阶段 4 | 阶段 4 | 待分配 | Backend / Web | 重要公告已读需要幂等处理 | 未开始 |
| 内容沉淀 | 相册与照片 | 班级照片沉淀 | 实现相册、照片上传、照片说明、照片评论、活动关联 | 内容基础、动态评论、活动模块 | 不在本文档估算 | 阶段 5 | 阶段 5 | 待分配 | Backend / Web | 图片权限、上传安全、存储成本需控制 | 未开始 |
| 情感连接 | 生日祝福 | 本月生日同学 | 实现本月生日同学、生日祝福、展示开关和祝福管理 | 资料与通讯录、内容基础 | 不在本文档估算 | 阶段 5 | 阶段 5 | 待分配 | Backend / Web | 禁止保存或展示出生年份和具体日期 | 未开始 |
| 内容治理 | 举报与处理 | 治理闭环 | 实现举报入口、举报列表、处理动作、内容隐藏/删除、用户限制/封禁 | 主要内容模块、操作日志基础 | 不在本文档估算 | 阶段 6 | 阶段 6 | 待分配 | Backend / Web | 被举报对象多类型处理复杂 | 未开始 |
| 管理后台 | 后台治理 | 管理工作台 | 整合用户审核、用户管理、内容管理、活动管理、举报处理、公告管理、日志查看 | 账号、业务模块、举报治理 | 不在本文档估算 | 阶段 2 起增量，阶段 6 完整 | 阶段 6 | 待分配 | Web / Backend | 管理权限必须严格隔离 | 未开始 |
| 审计日志 | 操作日志 | 可追溯治理 | 记录审核、删除、封禁、角色变更、举报处理、活动状态修改等关键动作 | 账号体系、权限隐私 | 不在本文档估算 | 阶段 2 起预埋 | 阶段 6 完整 | 待分配 | Backend | 日志不能记录敏感明文信息 | 未开始 |
| 系统通知 | 通知预留 | 系统提醒 | 预留或实现审核结果、公告、评论回复、活动状态等系统通知，不提供私信 | 账号、公告、动态、活动 | 不在本文档估算 | 阶段 7 | 阶段 7 | 待分配 | Backend / Web | 容易被误做成私信能力，需要严格边界 | 未开始 |
| 部署上线 | 部署与备份 | 上线保障 | 完成 Docker Compose、Nginx、MySQL、Redis、备份恢复、安全配置和上线验收 | 核心模块完成 | 不在本文档估算 | 阶段 8 | 阶段 8 | 待分配 | DevOps / Backend | 备份恢复若未演练，上线风险高 | 未开始 |
| 多语言/国际化 | 中文优先 | 多语言影响评估 | 第一版默认中文，数据库使用 utf8mb4；多语言不作为第一版目标 | 基础架构 | 不在本文档估算 | 阶段 1 起关注 | 阶段 8 验收 | 待分配 | 通用 | 中文、emoji、时区格式需正确处理 | 未开始 |
| 可访问性 | 基础可用性 | 移动端和语义 | 页面应考虑移动端显示、表单可用性、焦点和基础语义 | 前端工程 | 不在本文档估算 | 阶段 3 起关注 | 阶段 8 验收 | 待分配 | Web | 后期补可访问性成本较高 | 未开始 |
| 主题/暗黑模式 | 主题能力 | 主题预留 | 第一版不强制暗黑模式，但前端样式应避免硬编码过多颜色，便于后续扩展 | 前端工程 | 不在本文档估算 | 阶段 1 起关注 | 阶段 8 验收 | 待分配 | Web | UI 组件库选择会影响主题扩展 | 未开始 |
| 埋点/日志 | 工程日志 | 日志与审计 | 应用错误日志、权限拒绝日志、上传失败日志、操作日志需要脱敏 | 基础架构、操作日志 | 不在本文档估算 | 阶段 2 起预埋 | 阶段 8 验收 | 待分配 | 通用 | 日志泄露手机号、邮箱、token 等敏感信息 | 未开始 |
| 监控/告警 | 运行监控 | 基础可观测性 | 监控应用存活、错误率、数据库连接、磁盘容量、备份结果等 | 部署环境 | 不在本文档估算 | 阶段 8 | 阶段 8 | 待分配 | DevOps | 初版容易忽略备份失败和磁盘容量 | 未开始 |
| 安全/隐私 | 安全验收 | 权限与隐私回归 | 上线前验证未登录、待审核、封禁、匿名展示、活动实名、生日月份、联系方式隐私等规则 | 所有核心模块 | 不在本文档估算 | 阶段 8 | 阶段 8 | 待分配 | 通用 | 是本项目上线前最高优先级验收项 | 未开始 |
...
5. 权限、隐私、匿名展示、活动实名、生日月份、举报治理这几类规则应持续回归，不应只在最后一次性测试。
